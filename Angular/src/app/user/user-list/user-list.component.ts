  ...
  implements OnInit, AfterViewInit
{
  ...
  filterLogic() {
    this.sub$.sink = this.emilFilterCtl.valueChanges
    ...
    this.sub$.sink = this.fullNameFilterCtl.valueChanges
      .pipe(debounceTime(400), distinctUntilChanged())
      .subscribe((c) => {
        if (c) {
          const name = c.trim().split(' ');
          this.userResource.first_name = name[0];
          if (name.length > 1) this.userResource.last_name = name[1];
          this.userResource.skip = 0;
        } else {
    ...
    this.sub$.sink = this.phoneNumberSearchFilterCtl.valueChanges
      .pipe(debounceTime(400), distinctUntilChanged())
      .subscribe((c) => {
