...
export class ManageUserComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.sub$.sink = this.activeRoute.data.subscribe((data: { user: User }) => {
      if (data.user) {
        ...
        if (data.user.userAllowedIPs.length > 0) {
          for (let i = 0; i < data.user.userAllowedIPs.length - 1; i++) {
            this.addIP();
          }
        }
        this.userForm
          .get('userAllowedIPs')
          .patchValue(data.user.userAllowedIPs);
        this.user = data.user;
      } else {
        this.userForm
          .get('password')
  ...
  createBuildObject(): User {
    ...
    const user: User = {
      ...
      userAllowedIPs: (this.userAllowedIPs.value as UserAllowedIP[]).filter(
        (c) => c.ipAddress
      ),
    };
    return user;
